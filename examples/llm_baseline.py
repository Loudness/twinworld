"""Head-to-head: a local LLM vs twinworld-with-gate, on ARC and CausalARC.

Part A (always): the known-fitting ARC tasks plus a few more corpus tasks.
The LLM always answers; twinworld answers only when its confidence gate is high
...
Endpoint: TWINWORLD_LLM_URL (default http://localhost:11434/v1/chat/completions)
"""
...
from twinworld.baselines.llm import llm_predict, reachable
from twinworld.domains.arc import iter_tasks, load_task
...
if not reachable():
    sys.exit("LLM endpoint not reachable — set TWINWORLD_LLM_URL or start the server")
...
llm_right = twinworld_right = twinworld_wrong = abstained = unsolved = 0
for task in tasks:
    ...
    print(f"  {task.task_id}: LLM {'correct' if llm_ok else 'wrong  '} ({llm_ms:6.0f}ms) "
          f"| twinworld {ours:9s} ({our_ms:5.0f}ms)", flush=True)

print(f"\n  LLM    : {llm_right}/{len(tasks)} correct (always answers)")
print(f"  twinworld : {twinworld_right} correct, {twinworld_wrong} wrong, "
      f"{abstained} abstained, {unsolved} no fit")
print("  calibration: twinworld's wrong-when-confident count is the number to watch")
