...
from dowhat.mechanisms import All, Smallest
...
def test_negation_extends_the_solvable_language(except_largest_task):
    ...
    with pytest.raises(dowhat.UnsolvedTaskError):
        dowhat.model(except_largest_task, induction="never")
...
def test_pertinent_negatives_discriminate_hypotheses(small_ambiguous_task):
    ...
    by_colour = dowhat.model(
        small_ambiguous_task,
        primitives=[ObjectRule(ByColour(2), RecolourTo(5))],
        induction="never",
        max_depth=1,
    )
    robust = dowhat.compute(dowhat.identify(by_colour, PertinentNegative(max_cells=3)))
    (item,) = robust.items
    assert "bounded certificate" in item.narrative
    assert not item.metrics.applicable

    largest = dowhat.model(
        small_ambiguous_task,
        primitives=[ObjectRule(Largest(), RecolourTo(5))],
        induction="never",
        max_depth=1,
    )
    fragile = dowhat.compute(dowhat.identify(largest, PertinentNegative(max_cells=3)))
...
def test_pertinent_negative_finds_applicability_boundary(move_recolor_task):
    ...
    pn = dowhat.compute(dowhat.identify(rep, PertinentNegative(max_cells=1)))
    assert pn.items
...
def test_pertinent_negative_identify_errors(recolor_task):
    ...
    with pytest.raises(IdentificationError, match="max_cells"):
        dowhat.identify(rep, PertinentNegative(max_cells=0))
    with pytest.raises(IdentificationError, match="does not exist"):
        dowhat.identify(rep, PertinentNegative(on="test[7]"))
...
def test_negation_increases_underdetermination(recolor_task):
    ...
    fits = solve_all(recolor_task, dowhat.induce_rules(recolor_task), max_depth=1)
...
def test_asp_selectors_agree_with_python():
    ...
    from dowhat.asp import asp_select
