...
clear; the destination must have room) and *context-dependent effects* (the
block lands on top of whatever the destination holds, so the same move shifts
a block by different offsets in different states — provably inexpressible as
a translation rule). The core — representation, engine, all query types,
metrics, refuters, diagnosis — is used unchanged: this module supplies only
the perception adapter and the primitive library, which is the whole
generality claim.
"""
